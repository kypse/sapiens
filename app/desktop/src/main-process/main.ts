...
if (__DARWIN__) {
  app.on('open-file', async (event, path) => {
    ...
    Fs.stat(path, (err, stats) => {
      ...
      handleAppURL(
        `x-sapiens-client://openLocalRepo/${encodeURIComponent(path)}`
      )
    })
  })
}
...
if (process.env.SAPIENS_DESKTOP_DISABLE_HARDWARE_ACCELERATION) {
  log.info(
    `SAPIENS_DESKTOP_DISABLE_HARDWARE_ACCELERATION environment variable set, disabling hardware acceleration`
  )
...
app.on('ready', () => {
  ...
  setAsDefaultProtocolClient('x-sapiens-client')

  if (__DEV__) {
    setAsDefaultProtocolClient('x-sapiens-desktop-dev-auth')
  } else {
    setAsDefaultProtocolClient('x-sapiens-desktop-auth')
  }

  // Also support Desktop Classic's protocols.
  if (__DARWIN__) {
    setAsDefaultProtocolClient('sapiens-mac')
  } else if (__WIN32__) {
    setAsDefaultProtocolClient('sapiens-windows')
  }
